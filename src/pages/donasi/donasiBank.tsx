...
export default function DonasiBank() {
  const router = useRouter();
  const [data, setData] = useState<UserData>()

  useEffect(() => {
    if (router.query.formDataJson) {
      try {
        const parsedData = JSON.parse(router.query.formDataJson as string);
        setData(parsedData);
      } catch (error) {
        console.error("Error parsing JSON:", error);
      }
    }
  }, [router.query.formDataJson]);
  return (
        ...
        <div className="w-[459px] flex flex-col gap-[31px] mx-auto">
          <div className="mx-auto">
            <p className="text-neutral-700 text-lg">
              Transfer ke Nomor Rekening Berikut untuk berdonasi :
            </p>
          </div>
          <div className="flex flex-col items-center mx-auto">
            <p className="text-indigo-800 text-4xl font-semibold">BRI</p>
            <p className="text-neutral-700 text-4xl font-semibold">
              0000-01-000000-02-3
            </p>
          </div>
